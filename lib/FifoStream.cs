...
public class FifoStream 
{
    ...
    public FifoStream(int workBufferSize) 
    {
        ...
        _workBuffer = new byte[workBufferSize];
    ...
    public int Read(int count, byte[] outBuffer, int outBufferOffset) 
    {
        ...
        while(remaining > 0) 
        {
            var current = _buffer[0];
            var currentLength = current.data.Length - current.offset;
    ...
    {
        var length = Math.Min(_length, _workBuffer.Length);
        var dict = new Dictionary<string, string>();

        Read(length, _workBuffer, 0);
        var cursor = 0;
        int valueLength;
        string name;
        string value;
        ...
        while(cursor < length) 
        {
            var nameLength = ParseNameValueLength(ref cursor, length);
            if (nameLength == -1) 
            {
                break;
            }
            ...
            name = System.Text.Encoding.UTF8.GetString(_workBuffer, cursor, nameLength);
            cursor += nameLength;
            value = System.Text.Encoding.UTF8.GetString(_workBuffer, cursor, valueLength);
            cursor += valueLength;
    ...
    private int ParseNameValueLength(ref int cursor, int bufferSize) 
    {
        ...
        if (_workBuffer[cursor] <= 127) 
            return _workBuffer[cursor++];
        ...
        if (_isLittleEndian)
        {
            length = ((_workBuffer[cursor] & 0x7f) << 24) + 
                     (_workBuffer[cursor + 1] << 16) + 
                     (_workBuffer[cursor + 2] << 8) + 
                     _workBuffer[cursor + 3];
        } 
        else 
        {
            length = (_workBuffer[cursor] & 0x7f) + 
                     (_workBuffer[cursor + 1] >> 8) + 
                     (_workBuffer[cursor + 2] >> 16) + 
                     (_workBuffer[cursor + 3] >> 24);
        }
